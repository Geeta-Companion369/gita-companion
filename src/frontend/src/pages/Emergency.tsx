...
const VERSES = [
  ...
  },
  {
    ...
    meaning:
      "The soul is eternal, unborn — it is not slain when the body is slain.",
  ...
  },
  {
    ...
    krishnaWords:
      "This darkness will pass. Do not make a permanent decision in a temporary moment.",
  },
  {
    id: 4,
    ref: "BG 9.22",
  ...
  },
  {
    ...
    meaning:
      "You are dear to me — not because you are perfect, but because you came to me.",
  ...
  },
  {
    ...
    meaning:
      "Beyond this darkness is eternal light — my abode where there is no more suffering.",
  ...
  },
  {
    ...
    krishnaWords:
      "Do not worry. I will take you out of this immediately. Just follow my path. You are eternal. You are loved. You are not alone. Hare Krishna.",
  },
];

const CRISIS_VERSE_ORDER: Record<string, number[]> = {
...
interface CrisisCategory {
  ...
  desc: string;
  verseRef: string;
  showHelplineFirst?: boolean;
}

const CRISIS_CATEGORIES: CrisisCategory[] = [
  {
    id: "suicidal",
    icon: "🕯️",
    hindi: "जीवन का संकट",
...
const HELPLINE_CATEGORIES: HelplineCategory[] = [
  {
    id: "emergency",
...
// Crisis-specific relevant helplines (by crisis category id → helpline category ids)
const CRISIS_HELPLINE_MAP: Record<string, string[]> = {
...
// Quick-dial for domestic violence (shown immediately)
...
const INTERNATIONAL_HELPLINES: IntlHelpline[] = [
  // USA
  {
    country: "🇺🇸 USA",
...
function KrishnaChariot({ isVisible }: { isVisible: boolean }) {
  const [fluteDown, setFluteDown] = useState(false);
        ...
        >
          <div className="relative flex items-center justify-center">
            <motion.div
              ...
              style={{
                ...
                height: 80,
                background:
                  "radial-gradient(circle, oklch(0.82 0.32 54 / 0.35) 0%, transparent 70%)",
              }}
          ...
          <AnimatePresence>
            {fluteDown && (
              ...
              >
                I have set aside my flute. Right now, only you matter.
              </motion.p>
            )}
          </AnimatePresence>
        </motion.div>
...
function HelplineCard({ h }: { h: Helpline }) {
  ...
  return (
    <a
      href={isText ? undefined : `tel:${h.number.replace(/[\s\-()]/g, "")}`}
      ...
      aria-label={
        isText ? `${h.name}: ${h.number}` : `Call ${h.name} at ${h.number}`
      }
    >
      ...
      >
        {isText ? h.number : `📞 ${h.number}`}
      </span>
    </a>
  );
}

function HelplineCategoryBlock({ cat }: { cat: HelplineCategory }) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="flex items-center gap-2 font-body font-semibold text-sm">
        ...
        <span style={{ color: "oklch(0.62 0.26 32)", fontSize: "0.85rem" }}>
          {open ? "▲" : "▼"}
        </span>
      </button>
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="px-3 pb-3 grid gap-2">
              {cat.helplines.map((h) => (
                <HelplineCard key={h.name + h.number} h={h} />
              ))}
            </div>
          </motion.div>
        )}
...
function HelplineSection({
  ...
  // Which categories to show as priority for this crisis
  const priorityCategoryIds = crisisId
    ? (CRISIS_HELPLINE_MAP[crisisId] ?? ["emergency", "mental_health"])
    : ["emergency", "mental_health"];

  const priorityCategories = HELPLINE_CATEGORIES.filter((c) =>
  ...
  return (
    ...
    >
      ...
      >
        📡 All helplines are pre-loaded and available without internet
        connection. Tap any number to call.
      </p>
      ...
      {priorityCategories.map((cat) => (
        <HelplineCategoryBlock key={cat.id} cat={cat} />
      ))}

      {/* International quick-list */}
      <div
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  >
                    {h.name}
                    {h.note && (
                      <span style={{ color: "oklch(0.50 0.06 54 / 0.6)" }}>
                        {" "}
                        · {h.note}
                      </span>
                    )}
      ...
      {/* View all India helplines toggle */}
      <button
        type="button"
        ...
        style={{
          background: viewAll
            ? "oklch(0.20 0.10 268 / 0.6)"
            : "oklch(0.22 0.12 268 / 0.7)",
          color: "oklch(0.78 0.12 74)",
          border: "1px solid oklch(0.48 0.24 268 / 0.35)",
        }}
      >
        {viewAll
          ? "▲ Hide All India Helplines"
          : "▼ View All India Helplines (All Categories)"}
      </button>
      ...
      {viewAll && (
        ...
        >
          ...
          >
            All helplines work 24/7 unless noted. Tap any number to call.
          </p>
      ...
      >
        🔒 These numbers are pre-loaded and available without internet
        connection. Krishna sends help through many hands.
      </p>
    </motion.div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, oklch(0.38 0.18 50 / 0.9), oklch(0.28 0.14 44 / 0.85), oklch(0.35 0.16 52 / 0.9))",
          padding: "2px",
          boxShadow:
            "0 0 48px oklch(0.78 0.34 54 / 0.35), 0 8px 32px oklch(0.48 0.24 268 / 0.2), inset 0 1px 0 oklch(0.90 0.42 54 / 0.3)",
        }}
      >
        {/* Inner content */}
        <div
          ...
          style={{
            background:
              "linear-gradient(160deg, oklch(0.22 0.12 46 / 0.97) 0%, oklch(0.18 0.10 268 / 0.97) 60%, oklch(0.20 0.11 44 / 0.97) 100%)",
          }}
        >
          {/* OM + flower decorative header */}
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <span
              ...
              style={{
                ...
                textShadow:
                  "0 0 28px oklch(0.82 0.36 54 / 0.7), 0 0 56px oklch(0.78 0.34 52 / 0.4)",
                lineHeight: 1,
              }}
            >
              ॐ
          ...
          >
            You Are Not Alone — Krishna's Promise
          </h2>
          <p
          ...
          >
            {/* Prominent at-the-top story line */}
            <div
            ...
            <AnimatePresence>
              {expanded && (
                ...
                >
                  ...
                  >
                    That book saved this life. And so this app was born — not as
                    a business, not as a project, but as seva. As an offering
                    back to Krishna for the grace He showed. This app is our way
                    of making sure that the next soul sitting alone in a dark
                    hour, wondering if it&rsquo;s worth going on — finds
                    Krishna. Finds the Gita. Finds that still, small voice
                    saying:{" "}
                    <em style={{ color: "oklch(0.82 0.30 54)" }}>
                      &lsquo;I am here. I have always been here. I will never
                      leave you.&rsquo;
                    </em>
                  </p>
                </motion.div>
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, oklch(0.28 0.16 50 / 0.7), oklch(0.22 0.12 268 / 0.6))",
          ...
          >
            ...
            >
              Krishna came to this man through a stranger at a railway station.
              He came to you through this app. He never stops finding ways to
              reach his Arjuna. Never.
            </p>
...
function EmergencyAlertBanner() {
  return (
    <motion.div
      ...
      style={{
        background:
          "linear-gradient(135deg, oklch(0.25 0.16 24 / 0.92), oklch(0.20 0.13 20 / 0.9))",
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        >
          Krishna is with you right now. You are not alone.
        </p>
      </div>
      <span
...
function EntryScreen({
  ...
  return (
    ...
    >
      {/* Emergency alert banner — always at top */}
      <EmergencyAlertBanner />

      {/* Personal Story Card — prominent at top */}
      <PersonalStoryCard />
      ...
      >
        <div className="flex items-center justify-center gap-3 mb-5">
          <div
            style={{
              ...
              width: 60,
              background:
                "linear-gradient(to right, transparent, oklch(0.82 0.32 54 / 0.7))",
            }}
          />
          <span style={{ fontSize: "1.5rem", color: "oklch(0.82 0.32 54)" }}>
            🪔
          </span>
          <div
            style={{
              ...
              width: 60,
              background:
                "linear-gradient(to left, transparent, oklch(0.82 0.32 54 / 0.7))",
            }}
          />
        </div>
        <p
        ...
        <div
          ...
          style={{
            ...
            border: "1px solid oklch(0.82 0.32 54 / 0.3)",
            boxShadow:
              "0 0 32px oklch(0.48 0.24 268 / 0.2), inset 0 1px 0 oklch(0.82 0.32 54 / 0.1)",
          }}
        >
          <p
          ...
          >
            You are not alone. Whatever weight you carry — bring it to me. I
            have set aside everything. Right now, only you matter.
          </p>
          <p
            className="font-display italic mt-3 text-sm"
            style={{ color: "oklch(0.62 0.26 32 / 0.9)" }}
          >
            Hare Krishna — you are not alone.
          </p>
        </div>
        ...
        >
          What is weighing on your heart today, Arjun?
        </p>
        <p
          className="font-body text-sm"
          style={{ color: "oklch(0.65 0.08 60 / 0.7)" }}
        >
          Choose your battle, and Krishna will speak directly to you.
        </p>
      </motion.div>

      {/* Crisis category grid */}
      <div
      ...
      >
        {CRISIS_CATEGORIES.map((cat, idx) => (
          <motion.button
            ...
            style={{
              background: cat.showHelplineFirst
                ? "oklch(0.25 0.14 28 / 0.8)"
                : "oklch(0.16 0.08 268 / 0.75)",
              border: cat.showHelplineFirst
                ? "1px solid oklch(0.62 0.26 32 / 0.5)"
                : "1px solid oklch(0.48 0.24 268 / 0.4)",
...
function VerseScreen({
  ...
  const order = categoryId
    ? (CRISIS_VERSE_ORDER[categoryId] ?? DEFAULT_ORDER)
    : DEFAULT_ORDER;
  const orderedVerses = order.map((i) => VERSES[i]);

  const isDomestic = categoryId === "domestic";
  const [verseIdx, setVerseIdx] = useState(0);
  const [showHelplines, setShowHelplines] = useState(isDomestic);
  ...
  return (
    ...
    >
      ...
      {/* Krishna's chariot scene */}
      <KrishnaChariot isVisible={true} />

      {/* Domestic violence helplines first */}
      {isDomestic && showHelplines && (
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <div className="mb-5">
          ...
          >
            <motion.div
              ...
              style={{
                background:
                  "linear-gradient(to right, oklch(0.62 0.26 32), oklch(0.82 0.32 54))",
              }}
          ...
          >
            ...
            <div
              ...
              style={{
                ...
                border: "1px solid oklch(0.82 0.32 54 / 0.25)",
                boxShadow:
                  "0 0 40px oklch(0.48 0.24 268 / 0.2), inset 0 1px 0 oklch(0.82 0.32 54 / 0.1)",
              }}
            >
              <p
            ...
            {/* Krishna's personal words */}
            <div
              className="rounded-xl p-5 mb-5"
              style={{
                background:
                  "linear-gradient(135deg, oklch(0.20 0.12 46 / 0.9), oklch(0.18 0.10 268 / 0.9))",
                border: "1px solid oklch(0.62 0.26 32 / 0.4)",
            ...
            >
              <p
                ...
                style={{ color: "oklch(0.62 0.26 32 / 0.7)" }}
              >
                Krishna speaks to you directly —
              </p>
              <p
              ...
              >
                &ldquo;{currentVerse.krishnaWords}&rdquo;
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {verseIdx < 17 ? "Next Verse →" : "Complete ✦"}
              </motion.button>
            </div>
          </motion.div>
        </AnimatePresence>
      ...
      <AnimatePresence>
        {finished && (
          ...
          >
            ...
            >
              All 18 verses have spoken to your heart. Whenever you need me — I
              am here. Just say &ldquo;Hare Krishna.&rdquo;
            </p>
            ...
            {showHelplines && (
              <HelplineSection crisisId={categoryId} showAll={false} />
            )}
          </motion.div>
        )}
      </AnimatePresence>

      {/* Helplines toggle (non-domestic, not finished) */}
      {!finished && !isDomestic && (
        <div className="mt-6 text-center">
          ...
          >
            {showHelplines
              ? "Hide helplines"
              : "Need immediate human help? View helplines"}
          </button>
          <AnimatePresence>
            {showHelplines && (
              <motion.div
                ...
                transition={{ duration: 0.4 }}
              >
                <HelplineSection crisisId={categoryId} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
...
function EmergencyIntroBanner({ onEnter }: { onEnter: () => void }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      >
        {[
          ...
          >
            <span style={{ color: "oklch(0.82 0.32 54)", flexShrink: 0 }}>
              ✦
            </span>
            {item}
          </div>
      ...
      >
        "I shall deliver you. Do not fear." — BG 18.66
      </motion.p>
    </motion.div>
  );
}
...
export function EmergencyPage() {
  ...
  const [screen, setScreen] = useState<"entry" | "verses">("entry");
  ...
  return (
    ...
    >
      ...
      <AnimatePresence mode="wait">
        {showIntro && (
          <EmergencyIntroBanner key="intro" onEnter={handleEnterEmergency} />
        )}
      </AnimatePresence>
          ...
          >
            <motion.a
              ...
              style={{
                ...
                color: "oklch(0.10 0.06 28)",
                boxShadow:
                  "0 4px 32px oklch(0.62 0.26 32 / 0.5), 0 0 0 2px oklch(0.82 0.32 54 / 0.2)",
                letterSpacing: "0.03em",
              }}
            >
              <span>🙏</span>
              <span>Krishna, I need you</span>
            </motion.a>
          </div>
        </>
      )}
